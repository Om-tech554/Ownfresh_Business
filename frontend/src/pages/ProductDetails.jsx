    ...
    <>
      ...
      <div className="min-h-screen py-6 sm:py-10 px-3 sm:px-8 md:px-16 lg:px-20 bg-[#fafafa] pb-44 lg:pb-12">
        <div className="max-w-7xl mx-auto">
          ...
          {/* BREADCRUMB */}
          <div className="flex items-center gap-2 text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 sm:mb-8 overflow-x-auto no-scrollbar">
            <SLink to="/" className="hover:text-slate-800 whitespace-nowrap">Home</SLink>
            <span>/</span>
            ...
            <div className="lg:col-span-6 flex flex-col gap-3 sm:gap-4 lg:sticky lg:top-28">
              ...
              <div className="relative w-full h-[300px] xs:h-[360px] sm:h-[480px] bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-10 border border-slate-200/80 shadow-xs flex items-center justify-center overflow-hidden group">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 sm:p-2.5 bg-white/90 hover:bg-white text-slate-700 rounded-xl shadow-sm border border-slate-100 opacity-90 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity z-10 cursor-pointer"
                  title="Zoom Image"
                >
                  <Maximize2 size={15} />
                </button>
                ...
                {selectedVariant && (
                  <div className="absolute bottom-3 right-3 sm:bottom-4 sm:right-4 bg-slate-900/85 backdrop-blur-sm text-white px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-wider">
            ...
            <div className="lg:col-span-6 flex flex-col justify-center">
              ...
              <div className="flex items-center justify-between gap-4 mb-2">
                ...
                <div className="flex items-center gap-1.5 text-amber-500">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={14} className="fill-amber-500 stroke-amber-500" />
                  ))}
                  <span className="text-xs font-bold text-slate-800 ml-1">5.0</span>
                  <span className="text-xs text-slate-400 font-semibold">(Verified Quality)</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 mt-2 text-xs font-bold text-slate-400">
                <span>SKU: <strong className="text-slate-700">{selectedVariant?.sku || product.sku || "OF-OIL"}</strong></span>
                <span>•</span>
              ...
              {/* PRICE BOX */}
              <div className="mt-6 p-5 bg-white rounded-2xl border border-slate-200/80 shadow-xs flex items-baseline gap-4">
                <span className="text-4xl sm:text-5xl font-black text-[#1E971D] font-mono">
                  ...
                  {product.variants?.filter((v) => v.status === "Active" && !v.name?.toLowerCase().includes("gift") && !v.name?.toLowerCase().includes("eco packaging")).map((variant) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-xs font-bold font-mono mt-1 ${isSelected ? "text-white" : "text-[#1E971D]"}`}>
                          ₹{vPrice}
                        </span>
                      </button>
                    );
              ...
              <div className="mt-8 flex items-center gap-6">
                <div>
                  ...
                  <div className="flex items-center bg-white border border-slate-200 rounded-2xl p-1 shadow-xs">
                    <button
                      ...
                      className="w-10 h-10 flex items-center justify-center font-black text-slate-600 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer text-lg"
                    >
                      -
                    </button>
                    <span className="w-12 text-center font-black text-base text-slate-900">{selectedQty}</span>
                    <button
                ...
                <div className="flex-1 pt-6">
                  <span className="text-xs text-slate-400 font-bold block mb-1">Total Bottle Volume:</span>
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  ...
                  className="bg-slate-900 hover:bg-slate-800 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 disabled:opacity-50 cursor-pointer"
                >
                ...
                <button
                  ...
                  disabled={selectedVariant?.stockQuantity <= 0}
                  className="bg-[#1E971D] hover:bg-[#167a17] text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-md shadow-[#1E971D]/20 active:scale-95 disabled:opacity-50 cursor-pointer"
                >
                  <Zap size={18} /> Buy Now
                </button>
              </div>
              ...
              <div className="mt-10 grid grid-cols-3 gap-4 pt-8 border-t border-slate-200">
                <div className="flex flex-col items-center text-center p-3 bg-white rounded-2xl border border-slate-100">
                  <ShieldCheck size={20} className="text-[#1E971D] mb-1.5" />
                  <span className="text-[10px] font-black text-slate-800 uppercase tracking-wider">Lab Tested</span>
                  <span className="text-[9px] text-slate-400">Zero Chemicals</span>
                </div>
                <div className="flex flex-col items-center text-center p-3 bg-white rounded-2xl border border-slate-100">
                  <Truck size={20} className="text-[#1E971D] mb-1.5" />
                  <span className="text-[10px] font-black text-slate-800 uppercase tracking-wider">Fast Delivery</span>
                  <span className="text-[9px] text-slate-400">Safe Bottle Packing</span>
                </div>
                <div className="flex flex-col items-center text-center p-3 bg-white rounded-2xl border border-slate-100">
                  <Leaf size={20} className="text-[#1E971D] mb-1.5" />
                  <span className="text-[10px] font-black text-slate-800 uppercase tracking-wider">Stone Pressed</span>
                  <span className="text-[9px] text-slate-400">Nutrients Preserved</span>
                </div>
              </div>
          ...
          {fbtItem1 && (
            <div className="mt-14 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                ...
                {fbtCount >= 2 && (
                  <span className="bg-emerald-600 text-white text-xs font-black px-3.5 py-1.5 rounded-full uppercase tracking-wider self-start md:self-auto shadow-xs animate-pulse">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                <div className="lg:col-span-7 flex items-center justify-start gap-2 sm:gap-4 overflow-x-auto pb-2 no-scrollbar">
                  {/* Main item */}
                  <div className={`relative p-3 rounded-2xl border transition-all ${fbtMain ? "bg-white border-[#1E971D] shadow-sm ring-2 ring-[#1E971D]/10" : "bg-slate-100 border-slate-200 opacity-40"}`}>
                    <img
                      src={activeImage || product.image}
                      alt={displayName}
                      className="w-20 h-20 sm:w-24 sm:h-24 object-contain"
                    />
                    <span className="text-[9px] font-black uppercase text-center block mt-1 text-slate-700 truncate max-w-[80px]">
                      This Item
                    </span>
                  </div>
                  ...
                  {fbtItem1 && (
                    <>
                      <div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-500 shrink-0 font-black text-sm">
                        +
                      </div>

                      <div className={`relative p-3 rounded-2xl border transition-all ${fbtAdd1 ? "bg-white border-[#1E971D] shadow-sm ring-2 ring-[#1E971D]/10" : "bg-slate-100 border-slate-200 opacity-40"}`}>
                        <img
                  ...
                  {fbtItem2 && (
                    <>
                      ...
                      <div className={`relative p-3 rounded-2xl border transition-all ${fbtAdd2 ? "bg-white border-[#1E971D] shadow-sm ring-2 ring-[#1E971D]/10" : "bg-slate-100 border-slate-200 opacity-40"}`}>
                        <img
                ...
                <div className="lg:col-span-5 bg-slate-50 p-5 rounded-2xl border border-slate-200/80 flex flex-col justify-between">
                  <div className="space-y-2.5 mb-4 text-xs">
                    ...
                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                      <input
                        ...
                        className="w-4 h-4 rounded text-[#1E971D] accent-[#1E971D] cursor-pointer"
                      />
                    ...
                    {fbtItem1 && (
                      <label className="flex items-center gap-2.5 cursor-pointer select-none">
                        ...
                        <span className="font-black font-mono text-slate-900">₹{fbtPrice1}</span>
                      </label>
                    )}
                    ...
                    {fbtItem2 && (
                      <label className="flex items-center gap-2.5 cursor-pointer select-none">
                        ...
                        <span className="font-black font-mono text-slate-900">₹{fbtPrice2}</span>
                      </label>
                    )}
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-200/80 flex items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      disabled={fbtCount === 0}
                      className="px-5 py-3 bg-[#1E971D] hover:bg-[#167a17] text-white rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all shadow-md shadow-[#1E971D]/20 active:scale-95 disabled:opacity-50 cursor-pointer"
                    >
          ...
          <div className="mt-20">
            ...
            {/* Tab Contents */}
            <div className="mt-8 bg-white p-6 sm:p-10 rounded-3xl border border-slate-200/80 shadow-sm">
              {/* ── TAB 1: OVERVIEW & BENEFITS ── */}
              {activeTab === "overview" && (
                <div className="space-y-8">
                  {/* Highlight Header Card */}
                  <div className="p-6 bg-gradient-to-r from-emerald-50 via-slate-50 to-amber-50 rounded-2xl border border-emerald-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#1E971D] bg-white px-3 py-1 rounded-full border border-emerald-200 inline-block mb-1.5">
                        {oilInfo.badgeTitle}
                      </span>
                      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="bg-white p-3 rounded-xl border border-slate-200 text-center shadow-xs">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Smoke Point</span>
                        <span className="text-sm font-extrabold text-slate-900">{oilInfo.smokePoint}</span>
                      </div>
                      <div className="bg-white p-3 rounded-xl border border-slate-200 text-center shadow-xs">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Shelf Life</span>
                        <span className="text-sm font-extrabold text-slate-900">{oilInfo.shelfLife}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2">
                      <Sparkles size={14} className="text-[#1E971D]" /> Native Bioactives & Micronutrients
                    </h4>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                      {oilInfo.keyNutrients.map((nut, idx) => (
                        <div key={idx} className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-start gap-3">
                          <CheckCircle2 size={16} className="text-[#1E971D] shrink-0 mt-0.5" />
                          <span className="text-xs font-bold text-slate-800 leading-snug">{nut}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                    <div className="p-5 rounded-2xl bg-emerald-50/50 border border-emerald-100">
                      <div className="flex items-center gap-2 text-emerald-800 font-bold text-xs uppercase tracking-wider mb-2">
                        <Heart size={16} className="text-[#1E971D]" /> Fatty Acid & Lipid Composition
                      </div>
                    ...
                    <div className="p-5 rounded-2xl bg-amber-50/50 border border-amber-100">
                      <div className="flex items-center gap-2 text-amber-900 font-bold text-xs uppercase tracking-wider mb-2">
                        <Droplets size={16} className="text-amber-600" /> Aroma & Sensory Characteristics
                      </div>
                      <p className="text-xs text-slate-700 font-semibold leading-relaxed">
              ...
              {activeTab === "method" && (
                <div className="space-y-8">
                  <div className="max-w-3xl">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#1E971D] bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full inline-block mb-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    {oilInfo.steps.map((st, i) => (
                      <div
                        key={i}
                        className="relative bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow"
                      >
                        <div>
                          <span className="text-2xl font-black text-[#1E971D] tracking-tighter block mb-2 font-mono">
                            {st.step}
                          </span>
                          <h4 className="text-xs font-black uppercase tracking-wide text-slate-900 mb-2">
                            {st.title}
                          </h4>
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-200/60 flex items-center justify-between">
                          <CheckCircle2 size={14} className="text-[#1E971D]" />
                          <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Step {i + 1}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* Method Benchmarks */}
                  <div className="p-6 bg-slate-900 text-white rounded-2xl grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                    <div>
                      <span className="text-xl sm:text-2xl font-black text-[#EFDB27] block font-mono">14-16</span>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Low RPM Pressing</span>
                    </div>
                    <div>
                      <span className="text-xl sm:text-2xl font-black text-[#EFDB27] block font-mono">&lt; 40°C</span>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Zero-Heat Friction</span>
                    </div>
                    <div>
                      <span className="text-xl sm:text-2xl font-black text-[#EFDB27] block font-mono">100%</span>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Single-Origin Seeds</span>
                    </div>
                    <div>
                      <span className="text-xl sm:text-2xl font-black text-[#EFDB27] block font-mono">0%</span>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Chemical Solvents</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === "specs" && (
                <div className="space-y-8">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                      <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2">
                        <FileText size={14} className="text-[#1E971D]" /> Technical Specifications
                      </h4>
                      <div className="border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-100 text-xs">
                        <div className="grid grid-cols-2 p-3.5 bg-slate-50/60 font-semibold text-slate-700">
                          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Extraction Method</span>
                          <span className="font-bold text-slate-900">{oilInfo.extractionType}</span>
                        </div>
                        <div className="grid grid-cols-2 p-3.5 font-semibold text-slate-700">
                          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Raw Ingredient Source</span>
                          <span className="font-bold text-slate-900">{oilInfo.origin}</span>
                        </div>
                        <div className="grid grid-cols-2 p-3.5 bg-slate-50/60 font-semibold text-slate-700">
                          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Smoke Point</span>
                          <span className="font-bold text-emerald-700">{oilInfo.smokePoint}</span>
                        </div>
                        <div className="grid grid-cols-2 p-3.5 font-semibold text-slate-700">
                          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Shelf Life</span>
                          <span className="font-bold text-slate-900">{oilInfo.shelfLife}</span>
                        </div>
                        <div className="grid grid-cols-2 p-3.5 bg-slate-50/60 font-semibold text-slate-700">
                          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Packaging Type</span>
                          <span className="font-bold text-slate-900">Food-Grade UV Protected Airtight Bottle/Can</span>
                        </div>
                        <div className="grid grid-cols-2 p-3.5 font-semibold text-slate-700">
                          <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Preservatives / Additives</span>
                          <span className="font-bold text-emerald-700">Zero Added Chemicals, Zero Hexane</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2">
                        <Utensils size={14} className="text-[#1E971D]" /> Recommended Culinary & Wellness Uses
                      </h4>
                      <div className="space-y-3">
                        {oilInfo.bestFor.map((use, idx) => (
                          <div key={idx} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-white text-[#1E971D] border border-slate-200 flex items-center justify-center font-bold text-xs shrink-0 shadow-xs">
                              {idx + 1}
                            </div>
                            <span className="text-xs font-extrabold text-slate-800">{use}</span>
                          </div>
                        ))}
                      </div>

                      <div className="mt-4 p-4 rounded-2xl bg-amber-50 border border-amber-200/80">
                  ...
                  <div className="space-y-3 pt-2">
                    {(oilInfo.faqs || []).map((faq, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="border border-slate-200/80 rounded-2xl overflow-hidden bg-slate-50/50 transition-all"
                        >
                          <button
                            onClick={() => setOpenFaq(isOpen ? null : index)}
                            className="w-full p-4 sm:p-5 flex items-center justify-between gap-4 text-left font-bold text-xs sm:text-sm text-slate-900 hover:text-[#1E971D] transition-colors cursor-pointer"
                          >
                            <span className="flex items-center gap-3 font-black">
                              <HelpCircle size={18} className="text-[#1E971D] shrink-0" />
                              {faq.q}
                            </span>
                            {isOpen ? (
                          ...
                          <AnimatePresence>
                            {isOpen && (
                              <motion.div
                                ...
                                className="px-5 pb-5 text-xs text-slate-600 font-medium leading-relaxed border-t border-slate-100 bg-white pt-3"
                              >
                                {faq.a}
                              </motion.div>
                            )}
              ...
              {activeTab === "label" && product.labelImage && (
                <div className="flex flex-col items-center">
                  <h3 className="text-lg font-black uppercase tracking-wide text-slate-900 mb-6">
                    Official Packaging & Nutritional Bottle Label
                  </h3>
                  <div className="max-w-md w-full rounded-2xl overflow-hidden border border-slate-200 p-4 bg-slate-50">
              ...
              <div className="mt-10 pt-6 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-[11px] font-bold text-slate-500">
                <div className="flex items-center gap-2">
                  <ShieldCheck size={16} className="text-[#1E971D]" />
                  <span>100% Single-Origin Kernels</span>
                </div>
                <div className="flex items-center gap-2">
                  <RotateCcw size={16} className="text-[#1E971D]" />
                  <span>Traditional Granite Stone Kolhu</span>
                </div>
                <div className="flex items-center gap-2">
                  <Award size={16} className="text-[#1E971D]" />
                  <span>Certified Food-Grade Standards</span>
                </div>
                <div className="flex items-center gap-2">
                  <Leaf size={16} className="text-[#1E971D]" />
          ...
          {/* ── RELATED PRODUCTS ── */}
          <div className="mt-20">
            <h2 className="text-2xl font-black uppercase tracking-tight mb-8 text-slate-900">
