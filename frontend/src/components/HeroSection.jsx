...
const HeroSection = () => {
  ...
  return (
    <div className="w-full flex flex-col pt-0 group relative">
      ...
      {/* CAROUSEL CONTAINER */}
      <div className="relative w-full h-auto min-h-[85vh] md:min-h-[650px] lg:min-h-[700px] flex items-center justify-center overflow-hidden mx-auto border-b border-gray-100">
              ...
              {slide.layout === "full" && (
                <>
                  ...
                  <div className={`relative z-10 max-w-7xl mx-auto px-6 lg:px-16 w-full h-full flex flex-col justify-center items-start gap-4 transition-all duration-1000 delay-300 ${isActive ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0'}`}>
                    <p className="text-sm font-bold text-gray-500 uppercase tracking-[0.3em] mb-2 border-l-4 border-[#FFDD00] pl-3 shadow-sm">
                      {slide.subtitle}
                    </p>
                    <h1
                      className="text-3xl sm:text-5xl md:text-7xl font-black text-black leading-[1.1] tracking-tighter uppercase max-w-2xl drop-shadow-sm"
                      dangerouslySetInnerHTML={{ __html: slide.title }}
                    />
                    <p
                      className="text-xs sm:text-base md:text-lg font-medium text-gray-800 max-w-xl mt-4 mb-4 leading-relaxed"
                      dangerouslySetInnerHTML={{ __html: slide.description }}
                    />
                    <div className="flex flex-col sm:flex-row gap-4">
                      <button onClick={() => document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' })} className="px-8 py-4 bg-[#FFDD00] text-black text-sm font-bold uppercase tracking-widest hover:bg-[#181818] hover:text-[#FFDD00] transition-colors duration-300 shadow-xl border border-transparent hover:border-[#FFDD00]">
                        {slide.cta1}
                      </button>
              ...
              {slide.layout === "split" && (
                <div className={`relative z-10 max-w-[95%] mx-auto px-6 lg:px-12 w-full h-full flex flex-col-reverse md:flex-row items-center justify-center md:justify-between pt-12 pb-24 md:py-0`}>
                  ...
                  <div className={`w-full md:w-[35%] pr-4 md:pr-6 lg:pr-8 flex flex-col justify-center items-start gap-3 mt-8 md:mt-0 transition-all duration-1000 delay-300 ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                    <p className="text-xs sm:text-sm font-bold text-gray-500 uppercase tracking-[0.3em] mb-1 border-l-4 border-black pl-3 shadow-sm">
                      {slide.subtitle}
                    </p>
                    <h1
                      className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-black leading-[1.1] tracking-tighter uppercase max-w-2xl drop-shadow-sm"
                      dangerouslySetInnerHTML={{ __html: slide.title }}
                    />
                    <p
                      className="text-xs sm:text-sm md:text-base font-medium text-gray-600 max-w-xl mt-3 mb-4 leading-relaxed"
                      dangerouslySetInnerHTML={{ __html: slide.description }}
                    />
                    <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                      <button onClick={() => document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' })} className="w-full sm:w-auto px-8 py-4 bg-[#FFDD00] text-black border border-[#FFDD00] text-sm font-bold uppercase tracking-widest hover:bg-[#181818] hover:text-[#FFDD00] transition-colors duration-300 shadow-xl">
                        {slide.cta1}
                      </button>
                  ...
                  <div className={`w-full md:w-[65%] h-[300px] sm:h-[350px] md:h-[520px] lg:h-[580px] max-h-[580px] flex items-center justify-center transition-all duration-1000 delay-500 ${isActive ? 'scale-105 opacity-100' : 'scale-95 opacity-0'}`}>
                    <img
                      ...
                      className="w-full h-full max-h-full object-contain object-center drop-shadow-6xl"
                    />
                  </div>
                </div>
              )}
        ...
        <button
          onClick={prevSlide}
          className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 z-30 p-2 md:p-3 bg-white/80 hover:bg-[#FFDD00] text-black border border-gray-200 rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-x-4 md:-translate-x-8 group-hover:translate-x-0 shadow-lg"
        >
          <ChevronLeft size={24} />
        </button>
        <button
          onClick={nextSlide}
          className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 z-30 p-2 md:p-3 bg-white/80 hover:bg-[#FFDD00] text-black border border-gray-200 rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-4 md:translate-x-8 group-hover:translate-x-0 shadow-lg"
        >
        ...
        <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-30 flex gap-2">
          {slides.map((_, idx) => (
            <button
              ...
              className={`transition-all duration-300 rounded-full cursor-pointer ${idx === currentSlide ? 'w-8 h-2.5 bg-[#FFDD00] shadow-md border border-black/10' : 'w-2.5 h-2.5 bg-gray-300 hover:bg-gray-400'}`}
            />
          ))}
        </div>
      ...
      <div className="w-full bg-[#FFDD00] py-4 md:py-6 px-4 md:px-12 flex items-center justify-center shadow-inner z-20">
        <p className="text-center text-black font-extrabold uppercase tracking-widest text-xs md:text-sm lg:text-base selection:bg-[#181818] selection:text-[#FFDD00]">
