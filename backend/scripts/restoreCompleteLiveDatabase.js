...
async function restoreCompleteLiveDatabase() {
  ...
  let systemUser = await User.findOne({ email: "admin@myownfresh.com" });
  if (!systemUser) {
    systemUser = await User.create({
      fullName: "OwnFresh Official",
      userName: "ownfresh_admin",
      email: "admin@myownfresh.com",
      role: "admin",
      mobile: "+918999773438",
    ...
    console.log("  + Created System Admin User: admin@myownfresh.com");
  } else {
    console.log("  = Existing System Admin User found:", systemUser.email);
  }

  // --- 1. RESTORE ALL 7 CATEGORIES ---
  console.log("\n📂 1. Restoring Categories (7)...");
  await Category.deleteMany({});
  const categoriesData = [
    {
      name: "Coconut Oil",
      slug: "coconut-oil",
      description: "100% Pure Traditional Stone-Pressed Coconut Oil",
      image: "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1774199707/ownfresh_media/caqc2ep5nmgdf9cnknlu.png"
    },
    {
      name: "Groundnut Oil",
      slug: "groundnut-oil",
      description: "Wood & Stone Pressed Pure Groundnut (Peanut) Oil",
      image: "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1775112913/ownfresh_media/lo3psirlwzaqc5bsyfbx.png"
    },
    {
      name: "Mustard Oil",
      slug: "mustard-oil",
      description: "Traditional Kacchi Ghani Pungent & Pure Mustard Oil",
      image: "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1782888671/ownfresh_media/jc2gqhappjvdyfvrg6s4.png"
    },
    {
      name: "Sesame Oil",
      slug: "sesame-oil",
      description: "Cold Pressed Pure Sesame (Gingelly / Til) Oil",
      image: "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1782889051/ownfresh_media/jvpavxuq3hgbuxatjru0.png"
    },
    {
      name: "Safflower Oil",
      slug: "safflower-oil",
      description: "Heart Healthy Cold Pressed Safflower (Kardi) Oil",
      image: "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1782888819/ownfresh_media/ugty6jehznfi7mzqxlyz.png"
    },
    {
      name: "Sunflower Oil",
      slug: "sunflower-oil",
      description: "Natural Cold Pressed Unrefined Sunflower Oil",
      image: "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1782894879/ownfresh_media/ba8ioytkwzznt3vsk1x2.png"
    },
    {
      name: "Combo Value Packs",
      slug: "combo-value-packs",
      description: "Money-Saving Multi-Oil Health Combos & Value Bundles",
  ...
  // --- 2. RESTORE ALL 32 PRODUCTS & VARIANTS ---
  console.log("\n🛒 2. Restoring All 32 Products and Full Variants...");
  ...
  for (let idx = 0; idx < rawProducts.length; idx++) {
    ...
    const mainImage = p.images?.[0]?.src || targetCategory.image || "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1774199707/ownfresh_media/caqc2ep5nmgdf9cnknlu.png";
    ...
    if (oilPriceMap[catName]) {
      ...
      for (const sz of sizes) {
        ...
        const basePrice = pInfo.salePrice; // PDF base price
        const listingPrice = Math.round(basePrice * 1.20);
        const discountPercent = 10;
        const sellingPrice = Math.round(listingPrice * 0.90);
        await ProductVariant.create({
          ...
          stockQuantity: 100,
          status: pInfo.status || "Active"
        });
        variantCount++;
      }
    } else {
      // Combo Pack variants accurately derived from PDF bottle prices
      let comboBaseMrp = 1695;
      if (prodName.includes("Groundnut + Mustard + Sesame")) comboBaseMrp = 1695;
      else if (prodName.includes("Coconut + Safflower + Sunflower")) comboBaseMrp = 2310;
      else if (prodName.includes("Groundnut + Safflower + Sunflower")) comboBaseMrp = 1625;
      else if (prodName.includes("Groundnut") && prodName.includes("Sunflower") && prodName.includes("Sesame")) comboBaseMrp = 1655;
      else if (prodName.includes("Coconut") && prodName.includes("Groundnut") && prodName.includes("Sesame")) comboBaseMrp = 2410;
      else if (prodName.includes("250 ML Combo pack of 5")) comboBaseMrp = 1050;

      const comboVariants = [
        { name: "Standard Pack", basePrice: comboBaseMrp, shippingWeight: prodName.includes("250") ? 1.25 : 3.0 },
        { name: "Gift Box Edition", basePrice: comboBaseMrp + 150, shippingWeight: prodName.includes("250") ? 1.5 : 3.5 },
        { name: "Eco Packaging Pack", basePrice: comboBaseMrp, shippingWeight: prodName.includes("250") ? 1.25 : 3.0 }
      ];
      for (const cv of comboVariants) {
        const listingPrice = Math.round(cv.basePrice * 1.20);
        const discountPercent = 10;
        const sellingPrice = Math.round(listingPrice * 0.90);
        await ProductVariant.create({
          product: productDoc._id,
          ...
          price: listingPrice,
          salePrice: sellingPrice,
          shippingWeight: cv.shippingWeight,
          stockQuantity: 100,
          status: "Active"
        });
        variantCount++;
  ...
  // --- 3. RESTORE ALL 38 BLOGS ---
  console.log("\n📝 3. Restoring All 38 Educational Blogs...");
  ...
  if (fs.existsSync(blogsFile)) {
    ...
    for (const b of rawBlogs) {
      ...
      const slug = b.slug || "";
      const featuredImg = b._embedded?.['wp:featuredmedia']?.[0]?.source_url || 
                          b.jetpack_featured_media_url || 
                          "https://res.cloudinary.com/dkhq2wlwg/image/upload/v1774199707/ownfresh_media/caqc2ep5nmgdf9cnknlu.png";

      await Blog.create({
        title,
        slug,
        description: rawExcerpt || title,
        image: featuredImg,
        sections: [{ content: rawContent }],
        author: "Own Fresh Blogs",
        status: "LIVE",
        category: "Health & Wellness",
        focusKeyword: b.yoast_head_json?.schema?.['@graph']?.[0]?.name || title,
        searchDescription: rawExcerpt
      });
      console.log(`  + Created Blog: "${title.slice(0, 45)}..."`);
      blogCount++;
    }
    console.log(`  🎉 Restored ${blogCount} Blogs.`);
  }
